        ...
        <div className="card">
          ...
          {loading ? <div style={{ padding:40, textAlign:'center' }}><div className="spinner" style={{ margin:'0 auto' }} /></div> : (
            <table className="data-table">
              <thead><tr><th>Broker</th><th>Phone</th><th>Location</th><th>Speciality</th><th>Status</th><th></th></tr></thead>
              <tbody>
                {filtered.length === 0 && <tr><td colSpan="6" style={{ textAlign:'center', color:'var(--gray-400)', padding:32 }}>No brokers match your filters.</td></tr>}
                {filtered.map(b => (
                  <tr key={b.id}>
                    <td>
                    ...
                    <td><div>{b.phone}</div>{b.whatsapp && b.whatsapp !== b.phone && <div className="text-muted text-sm">WA: {b.whatsapp}</div>}</td>
                    <td>{b.location || '—'}</td>
